...
void connectAndSendData(char* encryptedPath, char* keyPath, char* port)
{
	...
	char encryptedString[BUFFERSIZE];
	...
	encrypted = fopen(encryptedPath, "r");
	key = fopen(keyPath, "r");

	if (encrypted != NULL && key != NULL)
		stringifyPlainTextAndKey(encrypted, key, encryptedString, keyString);
	...
	if (strcmp(getConfirmation, serverConfirmationCode) == 0) {
		...
		sendToServer(socketFD, encryptedString);
		...
		charsRead = recv(socketFD, decryptedText, sizeof(decryptedText) - 1, 0);
		if (charsRead < 0) {
			error("ERROR reading data from socket"); fflush(stdout);
		}
		else if (charsRead < sizeof(decryptedText) - 1) {
			//printf("CLIENT: There may be more data from socket.\n"); fflush(stdout);
		}

		//strcpy(encryptedText, completeMessage);
		printf("%s", decryptedText); fflush(stdout);
	}
	else {
...
void stringifyPlainTextAndKey(FILE *encrypted, FILE *key, char* encryptedString, char* keyString)
{
	...
	if (encrypted != NULL) {
		while ((c = fgetc(encrypted)) != '\n')
		{
			...
			encryptedString[encryptedCount++] = c;
		}
	}
